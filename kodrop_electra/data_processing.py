...
@Tokenizer.register("kodropelectra")
class BertDropTokenizer(Tokenizer):
    def __init__(self, pretrained_model: str):
        self.tokenizer = ElectraTokenizer.from_pretrained("monologg/koelectra-base-v2-discriminator")

    @overrides
    def tokenize(self, text: str) -> List[Token]:
        return [Token(token) for token in self.tokenizer.tokenize(text)]

@TokenIndexer.register("kodropelectra")
class BertDropTokenIndexer(WordpieceIndexer):
    def __init__(self,
                 pretrained_model: str,
                 max_pieces: int = 512) -> None:
        electra_tokenizer = ElectraTokenizer.from_pretrained("monologg/koelectra-base-v2-discriminator")
        super().__init__(vocab=electra_tokenizer.vocab,
                         wordpiece_tokenizer= electra_tokenizer.wordpiece_tokenizer.tokenize,
                         max_pieces=max_pieces,
                         namespace="electra",
                         separator_token="[SEP]")

@DatasetReader.register("kodropelectra")
class BertDropReader(DatasetReader):
    ...
    def _read(self, file_path: str):
        ...
        #num_dict = {"하나": 1, "second": 2, "third": 3, "fourth": 4, "fifth": 5, "1970s": 1970, "1950s":1950,
        #            "1960s":1960, "1980s":1980, "1990s":1990, "2000s":2000, "1930s":1930, "1940s":1940, "1920s":1920, "2010s":2010}
        instances = []
        for passage_id, passage_info in tqdm(dataset.items()):
            ...
            for token in word_tokens:
                #print(token)
                #if str(token) in ["하나", "second", "third", "fourth", "fifth", "1970s", "1950s", "1960s", "1980s", "1990s", "2010s", "2000s", "1940s", "1930s", "1920s"]:
                    #number = num_dict[str(token)]
                #else:
                wordpieces = self.tokenizer.tokenize(token.text)
                for word in wordpieces:
                    number = self.word_to_num(str(word))
    ...
    def text_to_instance(self,
        ...
        #print(len(question_passage_tokens))
